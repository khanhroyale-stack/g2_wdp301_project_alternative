  ...
  if (loading) {
    return (
      <div className={`animate-pulse rounded-organic border border-primary/5 bg-white p-6 ${className}`}>
        <div className="mb-4 h-5 w-48 rounded-full bg-primary/5" />
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between gap-4">
          ...
          {!compact && (
            <Link
              to="/goi-pro"
              className="shrink-0 rounded-pill border border-amber-200 bg-white px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-amber-700 shadow-sm transition-all hover:scale-105 hover:bg-amber-50 active:scale-95"
            >
              Gia hạn
            </Link>
          )}
        </div>
        <div className="relative z-10 mt-6 flex items-center gap-3 rounded-2xl border border-white/40 bg-white/60 p-4 text-sm font-bold text-amber-900 backdrop-blur-sm">
          ...
          {typeof activePosts === "number" && (
            <span className="ml-auto text-xs font-black uppercase tracking-widest text-amber-700/40">
  ...
  return (
    <div className={`rounded-organic border border-primary/5 bg-white p-6 shadow-apple-md ${className}`}>
      ...
      <div className="mt-8">
        ...
        <div className="h-2.5 overflow-hidden rounded-full border border-primary/5 bg-primary/5 p-0.5">
          <div
            className={`h-full rounded-full transition-all duration-1000 ${
              atLimit ? "bg-error" : "bg-primary shadow-[0_0_10px_rgba(42,90,59,0.3)]"
            }`}
            style={{ width: `${pct}%` }}
          />
        </div>
